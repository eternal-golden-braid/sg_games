...
class StackelbergSACSimulation:
    """
    Simulation class for the Stackelberg game using SAC with three robots.
    """
    def __init__(self, parameters):
        ...
        # Initialize agent
        self.agent = StackelbergSACAgent(
            state_dim=state_dim,
        ...
        self.updates_per_step = parameters.get('updates_per_step', 1)
    ...
    def _initialize_buffer(self, buffer_size, batch_size, state_dim, seed):
        """Initialize replay buffer for SAC."""
        ...
        return ReplayBuffer(buffer_size, batch_size, seed)
    ...
    def train(self, n_episodes=None, render_interval=None):
        ...
        for episode in range(n_episodes):
            ...
            for step in range(self.n_steps_per_episode):
                ...
                if len(self.buffer) >= self.batch_size:
                    for _ in range(self.updates_per_step):
                        ...
                        update_info = self.agent.update(experiences)
                        
                        leader_loss, follower1_loss, follower2_loss, alpha_value = update_info
            ...
            if episode_leader_losses:
                ...
                self.training_stats['alpha_values'].append(np.mean(episode_alpha_values))
        ...
        self.agent.save("checkpoints/sac_final")
    ...
    def _save_checkpoint(self, episode):
        ...
        self.agent.save(f"checkpoints/sac_episode_{episode}")
        ...
        checkpoint = {
            ...
            'alpha_values': self.training_stats['alpha_values']
        }
        
        try:
            os.makedirs('checkpoints', exist_ok=True)
            with open(f'checkpoints/sac_stats_ep{episode}.pkl', 'wb') as f:
    ...
    def visualize_training_stats(self):
        ...
        if self.training_stats.get('leader_losses'):
            ...
            # Plot alpha values (temperature parameter)
            if self.training_stats.get('alpha_values'):
                axes[2, 0].plot(self.training_stats['alpha_values'])
                axes[2, 0].set_xlabel('Episode')
                axes[2, 0].set_ylabel('Alpha')
                axes[2, 0].set_title('Temperature Parameter')
                axes[2, 0].grid(True)
