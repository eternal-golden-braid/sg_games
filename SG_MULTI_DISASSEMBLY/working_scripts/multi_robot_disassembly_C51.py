"""
Stackelberg C51 (Distributional Q-Learning) for Three-Robot Coordination
"""
import os
import numpy as np
import torch
...
class ReplayBuffer:
    """
    Standard replay buffer for storing and sampling experiences.
    """
    ...
    def add(self, experience):
        ...
        Parameters:
        - experience: Experience to add [state, a_leader, a_follower1, a_follower2, r_leader, r_follower1, r_follower2, next_state, done]
        """
...
class C51Network(nn.Module):
    ...
    """
    def __init__(self, input_dim, action_dim_leader, action_dim_follower1, action_dim_follower2, 
                 hidden_size=64, n_atoms=51, v_min=-10, v_max=10):
        ...
        self.n_atoms = n_atoms  # Number of atoms (discrete points) in the distribution
        self.v_min = v_min      # Minimum support value
        self.v_max = v_max      # Maximum support value
        ...
        # Output layer for value distribution
        self.output_layer = nn.Linear(
            hidden_size, 
            action_dim_leader * action_dim_follower1 * action_dim_follower2 * n_atoms
        )
    ...
    def forward(self, state):
        ...
        Returns:
        - Probability distributions over action combinations
        """
        ...
        # Reshape to [batch, leader_actions, follower1_actions, follower2_actions, n_atoms]
        logits = logits.view(batch_size, self.action_dim_leader, self.action_dim_follower1, 
                            self.action_dim_follower2, self.n_atoms)
    ...
    def get_q_values(self, probs):
        ...
        Parameters:
        - probs: Probability distributions [batch_size, leader_actions, follower1_actions, follower2_actions, n_atoms]
        
        Returns:
        - Q-values [batch_size, leader_actions, follower1_actions, follower2_actions]
        """
...
class StackelbergC51Agent:
    ...
    def __init__(self, state_dim, action_dim_leader, action_dim_follower1, action_dim_follower2, 
                 hidden_size=64, n_atoms=51, v_min=-10, v_max=10, device='cpu', learning_rate=1e-4,
                 gamma=0.9, epsilon=0.1, epsilon_decay=0.995, epsilon_min=0.01,
                 tau=0.01, update_every=10, buffer_size=10000, batch_size=32, seed=42):
        """
        ...
        - v_min, v_max: Range of the distribution support
        ...
        self.leader_online = C51Network(
            ...
            hidden_size, n_atoms, v_min, v_max).to(device)
    ...
    def compute_stackelberg_equilibrium(self, state):
        """
        Compute Stackelberg equilibrium using the current C51 networks.
        ...
        # Get probability distributions from networks
        leader_probs = self.leader_online(state_tensor)
        follower1_probs = self.follower1_online(state_tensor)
        follower2_probs = self.follower2_online(state_tensor)
        
        # Convert distributions to Q-values
        leader_q_values = self.leader_online.get_q_values(leader_probs)
        follower1_q_values = self.follower1_online.get_q_values(follower1_probs)
        follower2_q_values = self.follower2_online.get_q_values(follower2_probs)
    ...
    def project_distribution(self, rewards, next_probs, dones, agent_idx):
        """
        Project the distribution for the Categorical DQN (C51) algorithm.
        
        Parameters:
        - rewards: Batch of rewards
        - next_probs: Probability distributions from target network
        - dones: Batch of done flags
        - agent_idx: Index of the agent (0 for leader, 1 for follower1, 2 for follower2)
        ...
        # Extract rewards for the specific agent
        if agent_idx == 0:
            agent_rewards = rewards[:, 0]  # Leader rewards
        elif agent_idx == 1:
            agent_rewards = rewards[:, 1]  # Follower1 rewards
        else:
            agent_rewards = rewards[:, 2]  # Follower2 rewards
        ...
        for i in range(batch_size):
            if dones[i]:
                ...
                tz = torch.clamp(agent_rewards[i], self.v_min, self.v_max)
                ...
                for j in range(self.n_atoms):
                    ...
                    tz = torch.clamp(agent_rewards[i] + self.gamma * self.support[j], self.v_min, self.v_max)
                    bj = ((tz - self.v_min) / self.delta_z).floor().long()
                    l = (tz - self.v_min - bj * self.delta_z) / self.delta_z
                    u = 1.0 - l
    ...
    def learn(self):
        """
        Update the C51 networks using a batch of experiences.
        """
        ...
        rewards = torch.tensor([[exp[4], exp[5], exp[6]] for exp in experiences], dtype=torch.float).to(self.device)
        ...
        leader_probs_taken = leader_probs[batch_indices, actions_leader, actions_f1, actions_f2]
        follower1_probs_taken = follower1_probs[batch_indices, actions_leader, actions_f1, actions_f2]
        follower2_probs_taken = follower2_probs[batch_indices, actions_leader, actions_f1, actions_f2]
        ...
        with torch.no_grad():
            ...
            next_leader_q = self.leader_target.get_q_values(next_leader_probs)
            next_follower1_q = self.follower1_target.get_q_values(next_follower1_probs)
            next_follower2_q = self.follower2_target.get_q_values(next_follower2_probs)
            ...
            next_leader_q_np = next_leader_q.cpu().numpy()
            next_follower1_q_np = next_follower1_q.cpu().numpy()
            next_follower2_q_np = next_follower2_q.cpu().numpy()
            
            for b in range(self.batch_size):
                # Find Stackelberg equilibrium for this batch sample
                best_leader_value = float('-inf')
                ...
                for a_l in range(self.action_dim_leader):
                    ...
                    for _ in range(5):
                        f1_best_response = np.argmax(next_follower1_q_np[b, a_l, :, f2_action])
                        f2_best_response = np.argmax(next_follower2_q_np[b, a_l, f1_best_response, :])
                    ...
                    leader_value = next_leader_q_np[b, a_l, f1_action, f2_action]
            ...
            # Extract target distributions for Stackelberg actions
            next_leader_dist = next_leader_probs[batch_indices, next_leader_actions, next_f1_actions, next_f2_actions, :]
            next_follower1_dist = next_follower1_probs[batch_indices, next_leader_actions, next_f1_actions, next_f2_actions, :]
            next_follower2_dist = next_follower2_probs[batch_indices, next_leader_actions, next_f1_actions, next_f2_actions, :]
            
            # Project distributions for each agent
            projected_leader_dist = self.project_distribution(rewards, next_leader_dist, dones, 0)
            projected_follower1_dist = self.project_distribution(rewards, next_follower1_dist, dones, 1)
            projected_follower2_dist = self.project_distribution(rewards, next_follower2_dist, dones, 2)
        
        # Compute KL divergence loss for each agent
...
class StackelbergC51Simulation:
    """
    Simulation class for the Stackelberg game using C51 for three robots.
    """
    def __init__(self, parameters):
        """
        ...
        # Initialize agent
        self.agent = StackelbergC51Agent(
            state_dim=state_dim,
    ...
    def train(self, n_episodes=None, render_interval=None):
        ...
        for episode in range(n_episodes):
            ...
            if n_episodes >= 1000 and episode > 0 and episode % 200 == 0:
                ...
                self.agent.save(f"checkpoints/c51_episode_{episode}")
                ...
                try:
                    ...
                    with open(f'checkpoints/c51_stats_ep{episode}.pkl', 'wb') as f:
        ...
        # Save final model
        self.agent.save("checkpoints/c51_final")
...
def run_stackelberg_c51_simulation(n_episodes=200, render_interval=None):
    """
    Run a simulation using the Stackelberg C51 implementation.
    ...
    parameters = {
        ...
        'epsilon_min': 0.01,
        'tau': 0.01,
        'n_atoms': 51,      # Number of atoms for C51
        'v_min': -10,       # Minimum value for C51 support
        'v_max': 10         # Maximum value for C51 support
    }
    
    # Create the simulation
    sim = StackelbergC51Simulation(parameters)
    
    # Train the agent
    print(f"Starting Stackelberg C51 training with {n_episodes} episodes...")
...
if __name__ == "__main__":
    # Run the simulation
    simulation, eval_results = run_stackelberg_c51_simulation(n_episodes=1000)