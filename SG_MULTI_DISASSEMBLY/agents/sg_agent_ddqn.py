...
class StackelbergRainbowAgent(BaseAgent):
    ...
    def save(self, path: str) -> None:
        ...
        # For each atom in the distribution
        for i in range(batch_size):
            if dones[i]:
                # For terminal states, only immediate reward matters
                ...
                projected_distribution[i, b] += (1 - l) * next_distribution[i]
                
                if b < self.n_atoms - 1:
                    projected_distribution[i, b + 1] += l * next_distribution[i]
            else:
                # For non-terminal states, apply Bellman update
