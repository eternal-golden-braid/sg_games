...
class StackelbergSACAgent(BaseAgent):
    ...
    def act(self, state: np.ndarray, action_masks: Optional[Dict[str, np.ndarray]] = None, 
        ...
        if action_masks is not None:
            ...
            # If no masks provided, create masks that only allow actions within state bounds
            leader_mask = torch.zeros(self.action_dim_leader, dtype=torch.bool, device=self.device)
            follower1_mask = torch.zeros(self.action_dim_follower1, dtype=torch.bool, device=self.device)
            follower2_mask = torch.zeros(self.action_dim_follower2, dtype=torch.bool, device=self.device)
            
            # Allow actions within state bounds and the idle action (-1)
            leader_mask[0:state_dim] = True  # Allow actions 0 to state_dim-1
            leader_mask[-1] = True  # Allow idle action (-1)
            follower1_mask[0:state_dim] = True
            follower1_mask[-1] = True
            follower2_mask[0:state_dim] = True
            follower2_mask[-1] = True
        
        # Get action probabilities and sample actions
        with torch.no_grad():
            # Get action probabilities from actors
            leader_probs, _, _ = self.leader_actor(state_tensor)
            follower1_probs, _, _ = self.follower1_actor(state_tensor)
            follower2_probs, _, _ = self.follower2_actor(state_tensor)
            
            # Apply action masks
            leader_probs = leader_probs.masked_fill(~leader_mask, float('-inf'))
            follower1_probs = follower1_probs.masked_fill(~follower1_mask, float('-inf'))
            follower2_probs = follower2_probs.masked_fill(~follower2_mask, float('-inf'))
            
            # Convert to probabilities using softmax
            leader_probs = F.softmax(leader_probs, dim=-1)
            follower1_probs = F.softmax(follower1_probs, dim=-1)
            follower2_probs = F.softmax(follower2_probs, dim=-1)
            ...
            leader_action = leader_dist.sample().item()
            follower1_action = follower1_dist.sample().item()
            follower2_action = follower2_dist.sample().item()
            
            # Convert to actual action values (-1 for idle, 0 to state_dim-1 for valid actions)
            leader_action = -1 if leader_action == self.action_dim_leader - 1 else leader_action
            follower1_action = -1 if follower1_action == self.action_dim_follower1 - 1 else follower1_action
            follower2_action = -1 if follower2_action == self.action_dim_follower2 - 1 else follower2_action
